...
function getItemsPerView() {
    if (window.innerWidth <= 1080) {
        return { itemsPerView: 2, itemsToScroll: 2 }; // Hiển thị 1 mục và dịch chuyển 1 mục khi màn hình nhỏ hơn hoặc bằng 700px
    }
    return { itemsPerView: 4, itemsToScroll: 2 }; // Hiển thị 4 mục và dịch chuyển 2 mục khi màn hình lớn hơn 700px
}

function scrollCarousel(direction) {
    const carousel = document.querySelector('.ss .carousel-container .carousel');
    const items = document.querySelectorAll('.ss .carousel-container .carousel .carousel-item');
    const totalItems = items.length;
    const { itemsPerView, itemsToScroll } = getItemsPerView();

    currentIndex += direction * itemsToScroll;
...
// Sự kiện để xử lý khi thay đổi kích thước màn hình
window.addEventListener('resize', function() {
    // Reset lại vị trí của carousel
    currentIndex = 0;
    scrollCarousel(0);
});
